// 可播种确定性噪声：mulberry32 随机 + 2D/3D Simplex + fbm
// 纯计算模块，可在 Node 中测试。
...
export function fbm3(noise, x, y, z, octaves, lacunarity = 2, gain = 0.5) {
  let sum = 0, amp = 1, freq = 1, max = 0;
  for (let i = 0; i < octaves; i++) {
...
// 山脊噪声（尖锐山脉）
export function ridged(noise, x, y) {
  const n = 1 - Math.abs(noise.noise(x, y));
  return n * n;
}
